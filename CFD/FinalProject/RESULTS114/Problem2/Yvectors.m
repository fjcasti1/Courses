function [a,b,c,d] = Yvectors(Y,M,N,dt,t,step,Q)
...
Yinlet2=1;
Yinlet3=0;
% [Yinlet1,Yinlet2,Yinlet3]=Yin(t);

dx=alpha*dt/hx^2;
dy=alpha*dt/hy^2;
d1=dx/2;
d2=dy/2;
x=linspace(-5*hx/2,Lx+5*hx/2,M+6);
y=linspace(-5*hy/2,Ly+5*hy/2,N+6);
if step==1
    a = -d1*ones(M*N,1);
    b = (1+2*d1)*ones(M*N,1);
    c = -d1*ones(M*N,1);
